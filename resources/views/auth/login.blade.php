<x-app-layout>
    ...
    <div class="relative bg-primary text-white h-64 flex items-center justify-center bg-cover bg-center" style="background-image: url('assets/images/page-banner.jpg');">
                ...
                <div class="w-full lg:w-1/2">
                    <div class="bg-gray-50 border rounded-lg p-8 shadow-sm">
                        ...
                        <form action="{{ route('login') }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <input id="email" type="text" placeholder="Email *" :value="old('email')" name="email" class="w-full border p-3 rounded focus:outline-none focus:border-primary transition" required autofocus autocomplete="username"/>
                                @error('email')
                            ...
                            <div>
                                <input id="password" type="password" placeholder="Password" name="password" class="w-full border p-3 rounded focus:outline-none focus:border-primary transition" required autocomplete="current-password"/>
                                @error('password')
                                    <p class="text-sm text-red-600 mt-1 font-semibold">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" id="remember_me" name="remember" class="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary cursor-pointer" />
