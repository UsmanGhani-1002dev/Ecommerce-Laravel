...
<!-- Main Content Start -->
    ...
    <main class="flex-1 overflow-y-auto p-6 bg-gray-100">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <a href="{{ route('category.create') }}" class="bg-primary hover:bg-blue-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2 shadow-sm">
                <i class="fa-solid fa-plus"></i> Add New Category
            </a>
        </div>

        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
            <form action="{{ route('admin.category') }}" method="GET" class="flex flex-col md:flex-row gap-4 justify-between">
                <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                    ...
                    @if (request('search') || request()->filled('status'))
                        <a href="{{ route('admin.category') }}" class="text-gray-500 hover:text-gray-700 text-sm transition flex items-center gap-2 ">
                            <i class="fa-solid fa-xmark"></i> Clear
                        </a>
                    @endif
                </div>

                <button type="submit" class="hidden bg-primary hover:bg-secondary text-white px-5 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2 shadow-sm"><i class="fa-solid fa-filter"></i> Apply Filter</button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        @forelse($categories as $category)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    @if($category->parent)
                                        <span class="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-medium">{{ $category->parent->name }}</span>
                                    @else
                                        <span class="text-gray-400 font-normal">— (Main)</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('category.edit',$category->id) }}" class="w-8 h-8 rounded-full hover:bg-gray-100 text-blue-500 transition flex items-center justify-center" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{ route('category.delete', $category->id) }}" method="POST" id="delete-form-{{ $category->id }}">
                                            @csrf
                                            @method('DELETE')   
                                            <button type="button" class="w-8 h-8 rounded-full hover:bg-gray-100 text-red-500 transition flex items-center justify-center" title="Delete" onclick="deleteCategory(this, '{{ $category->name }}', '{{ $category->id }}')">
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <a href="{{ route('category.create') }}" class="mt-4 text-primary hover:underline text-sm font-medium">
            ...
            @if($categories->hasPages())
                <span class="text-sm text-gray-500">Showing <span class="font-bold text-gray-700">{{ $categories->firstItem() }}</span> to <span class="font-bold text-gray-700">{{ $categories->lastItem() }}</span> of <span class="font-bold text-gray-700">{{ $categories->total() }}</span> products</span>
...
<script>
    ...
    confirmBtn.addEventListener('click', function() {
        if(categoryIdToDelete){
            const deleteForm = document.getElementById(`delete-form-${categoryIdToDelete}`);
            if(deleteForm){
                deleteForm.submit();
            }
        }
