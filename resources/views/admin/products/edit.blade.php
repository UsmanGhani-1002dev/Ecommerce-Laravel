        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                ...
                <form action="{{ route('product.delete',['id' => $product->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition shadow-sm">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div>
                                ...
                                <input type="text" id="product-name" name="name" value="{{old('name', $product->name) }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Slug</label>
                                <input type="text" id="product-slug" name="slug" value="{{old('slug', $product->slug) }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm bg-gray-50">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Short Description</label>
                                <textarea id="short_description" name="short_description" rows="3" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm">{{ old('short_description', $product->short_description) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Information</label>
                                <textarea id="information" name="information" rows="3" placeholder="Information about product..." class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm">{{ old('information', $product->information) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                <textarea id="description" name="description" rows="16" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm">{{ old('description', $product->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-2 pt-2">
                                <input type="checkbox" id="featured" name="featured" value="1" {{ $product->featured == '1' ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
                        ...
                        @if($product->image)
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div class="existing-image-wrapper relative group h-24 bg-gray-100 rounded border overflow-hidden">
                                    <img src="{{ asset('/uploads/products/thumbnails/'.$product->image) }}" class="w-full h-full object-contain" alt="{{ $product->name }}">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        @foreach(explode(',', $product->images_gallery) as $gallery)
                            <div class="existing-image-wrapper relative group h-20 bg-gray-100 rounded border overflow-hidden">
                                <img src="{{ asset('/uploads/products/thumbnails/'.$gallery) }}" class="w-full h-full object-contain">
                                <div class="remove-existing-btn absolute inset-0 bg-black bg-opacity-50 hidden group-hover:flex items-center justify-center cursor-pointer text-white transition-opacity" data-input-name="deleted_gallery_images[]" data-image="{{ $gallery }}">
                                    <i class="fa-solid fa-trash pointer-events-none"></i>
                                </div>
                            </div>
                        @endforeach
