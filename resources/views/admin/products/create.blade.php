        ...
        <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-r-lg shadow-sm">
                    <p class="font-bold text-red-700 text-sm mb-1"><i class="fa-solid fa-triangle-exclamation mr-1"></i> Please fix the following errors:</p>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="product-name" name="name" placeholder="e.g. Jackets" value="{{ old('name') }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm" required>
                                @error('name')
                            ...
                            <div>
                                ...
                                <input type="text" id="product-slug" name="slug" placeholder="e.g. jackets" value="{{ old('slug') }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm bg-gray-50" required>
                                @error('slug')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="information" name="information" rows="3" placeholder="Information about product..." class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm">{{ old('information') }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                <textarea id="description" name="description" rows="16" placeholder="Detailed description..." class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-sm">{{ old('description') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number"  id="regular_price" name="regular_price" value="{{ old('regular_price') }}" placeholder="0.00" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm" required>
                                @error('regular_price')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="SKU" name="SKU" placeholder="Product SKU" value="{{ old('SKU') }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm" required>
                                @error('SKU')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                                ...
                                <select id="stock_status" name="stock_status" value="{{ old('stock_status') }}" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm bg-white">
                            ...
                            <div>
                                ...
                                <input type="number" id="quantity" name="quantity" value="{{ old('quantity') }}"  placeholder="Total items" class="w-full border px-4 py-2 rounded-lg focus:outline-none focus:border-primary text-sm" required>
                                @error('quantity')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <select id="status" name="status" class="border rounded text-sm px-6 py-1 bg-white focus:outline-none">
                                    <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>Draft</option>
                                    <option value="1" {{ old('status') == '1' || old('status') === null ? 'selected' : '' }}>Published</option>
                                </select>
                            </div>
                            <div class="flex items-center gap-2 pt-2">
                                <input type="checkbox" id="featured" name="featured" value="1" {{ old('featured') ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <label for="product-image" id="single-upload-label" class="block border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition cursor-pointer">
                            ...
                            <input type="file" id="product-image" name="image" value="{{ old('image') }}" class="hidden" accept="image/png, image/jpeg, image/jpg, image/webp">
                        </label>
                        @error('image')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <label for="product-images" class="block border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition cursor-pointer">
                            ...
                            <input type="file" id="product-images" name="images[]" value="{{ old('images') }}" class="hidden" multiple accept="image/png, image/jpeg, image/jpg, image/webp">
                        </label>
                       @if($errors->has('images') || $errors->has('image.*'))
                            <span class="text-red-500 text-sm mt-1">{{ $errors->first('images') ?: $errors->first('image.*') }}</span>
                        @endif
