...
<main class="flex-1 overflow-y-auto p-6 bg-gray-100">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <a href="{{ route('brands.create') }}" class="bg-primary hover:bg-secondary text-white px-5 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2 shadow-sm">
    ...
    @if (session('success'))
        <div id="success-message" class="bg-green-50 border-l-4 border-green-500 p-4 mb-4">
            {{ session('success') }}
        </div>
    @endif  
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('brands.edit', $brand->id) }}"  class="w-8 h-8 rounded-full hover:bg-gray-100 text-blue-500 transition flex items-center justify-center" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('brands.delete', $brand->id) }}" method="POST" id="delete-form-{{ $brand->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="w-8 h-8 rounded-full hover:bg-gray-100 text-red-500 transition flex items-center justify-center" title="Delete" onclick="deleteBrand(this, '{{ $brand->name }}', '{{ $brand->id }}')">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <a href="{{ route('brands.create') }}" class="mt-4 text-primary hover:underline text-sm font-medium">
        ...
        @if($brands->hasPages())
            <span class="text-sm text-gray-500">Showing <span class="font-bold text-gray-700">{{ $brands->firstItem() }}</span> to <span class="font-bold text-gray-700">{{ $brands->lastItem() }}</span> of <span class="font-bold text-gray-700">{{ $brands->total() }}</span> brands</span>
...
<script>
    ...
    confirmBtn.addEventListener('click', function() {
        if(brandIdToDelete){
            const deleteForm = document.getElementById(`delete-form-${brandIdToDelete}`);
            if(deleteForm){
                deleteForm.submit();
            }
        }
